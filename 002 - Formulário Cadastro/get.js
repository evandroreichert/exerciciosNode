const enviarGET = document.querySelector('#enviarGET')

enviarGET.addEventListener('click', async () => {
    ...
    let response = await fetch(`http://localhost:3000/aluno?nNome=${nome}&nSobrenome=${sobrenome}&nNascimento=${dataNasc}&nEndereco=${endereco}&nMae=${nomeMae}&nPai${nomePai}&nEmail${email}&nTel${telefone}&nSerie=${serie}&sexo=${sexo}&nAuditiva=${auditiva}&nMotora=${motora}&nVisual=${visual}&nParalisia=${paralisia}&nAutismo=${autismo}&nForo=${foro}&nAprendizagem=${aprendizagem}&nHabilidades=${habilidades}`)
